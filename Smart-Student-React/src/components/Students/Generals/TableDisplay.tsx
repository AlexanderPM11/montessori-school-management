...
interface Props {
  table: Table<Student>;
  toLink: string;
  isParentsView?: boolean; // Optional prop to indicate if it's a parents view
}

export const TableDisplay: FC<Props> = ({ table, toLink, isParentsView }) => {
      ...
      <tbody className="divide-y divide-gray-100 overflow-y-auto">
        {table.getRowModel().rows.map((row) => (
          <tr
            onDoubleClick={() =>
              navigate(
                `${toLink}?idStudent=${row.original.id}&isParentsView=${isParentsView}`
              )
            }
            key={row.id}
