...
interface Roomstate {
    ...
    // Actions
    getAllRoomsAsync: () => Promise<ApiResponse<Room[]>>;
...
const creatorRoom: StateCreator<Roomstate> = (set, get) => ({
    searchTerm: "",
    loading: true,
    getRoom: false,
    currentPage: 0,
    ...
    getAllRoomsAsync: async (): Promise<ApiResponse<Room[]>> => {
        try {
            let dataRooms: Room[];
...
export const useRoomsStore = create<Roomstate>()(creatorRoom);
